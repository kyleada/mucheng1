...
public class PointsMatrix {
	private final String vertexShaderCode =
			...
			"uniform mat4 uMVPMatrix;" +
					...
					"  gl_Position = vPosition * uMVPMatrix; gl_PointSize = 16.0;" + "}";
...
//	private final ShortBuffer drawListBuffer, drawLineListBuffer, cubeListBuffer[] = new ShortBuffer[5];
	...
	float color_rect[] = {0XDD / 255.0f, 0XB3 / 255.0f, 0X4D / 255.0f, 1.0f};
	float color_rect2[] = {0XDD / 255.0f, 0X00 / 255.0f, 0X00 / 255.0f, 1.0f};
	float color_megvii[][] = {
			{0f, 0f, 0f, 1.0f},
			{1.0f, 1.0f, 1.0f, 1.0f},
			{1f, 0f, 0.f, 1.0f}, //red
	...
	// 画人脸中心点,不止一个
	public ArrayList<FloatBuffer> faceRectLeftTops = new ArrayList<FloatBuffer>();
...
//                for (int j = 0; j < triangleVBList.size(); j++) {
				for (int j = AppConfig.NOSE_CENTER_INDEX; j < AppConfig.NOSE_CENTER_INDEX + 1; j++) {
...
//        synchronized (this){
//			GLES20.glUniform4fv(mColorHandle, 1, color_rect2, 0);
//			for (int i = 0; i < faceRectLeftTops.size(); i++){
//				FloatBuffer fb = faceRectLeftTops.get(i);
//				if(fb != null){
//					GLES20.glVertexAttribPointer(mPositionHandle, 3, GLES20.GL_FLOAT, false, 0, fb);
//					GLES20.glDrawArrays(GLES20.GL_POINTS, 0, 1);
//				}
