...
import com.mucheng.audio.NarratorSpeak;
import com.mucheng.audio.PlayManager;
import com.mucheng.common.AppConfig;
import com.mucheng.image.SGLRender;
...
public class OpenglActivity extends Activity
	...
	private static final String TAG = "OpenglActivity kyle";
    ...
    private void init() {
        ...
        isDebug = getIntent().getBooleanExtra("isdebug", true);
		...
		sglRender=new SGLRender(mGlSurfaceView);
		sglRenderBackGround = new SGLRender(mGlSurfaceView);
		try {
			sglRender.setImage(BitmapFactory.decodeStream(getResources().getAssets().open("texture/fengj.png")));
			sglRender.setNeedMatrixVary(true);
			sglRenderBackGround.setImage(BitmapFactory.decodeStream(getResources().getAssets().open("texture/air_hockey_surface.png")));
			sglRenderBackGround.setNeedMatrixVary(false);
		} catch (IOException e) {
			e.printStackTrace();
		}
    }
        ...
        mHandler.post(new Runnable() {
            ...
            public void run() {
                ...
                if (faces != null) {
					...
					ArrayList<FloatBuffer> pointsFaceLeftTop = new ArrayList<FloatBuffer>();
                    confidence = 0.0f;

                    if (faces.length >= 0) {
						if(faces.length == 1 && faceCount != faces.length){
							faceCount = faces.length;
							Log.d(TAG, "detect face 1");
							//丢失了一个人
						}else if(faces.length == 2 && faceCount != faces.length){
							faceCount = faces.length;
							Log.d(TAG, "detect face 2");
							//剧情可以重新恢复
						}
                        for (int c = 0; c < faces.length && c < AppConfig.MAX_FACE_COUNT; c++) {
                            if (is106Points)
                            ...
                            for (int i = 0; i < faces[c].points.length; i++) {
								...
								if(i == AppConfig.NOSE_CENTER_INDEX){
									noseCenterX = x;
//									Log.d(TAG, "detect nose center original x: " + faces[c].points[i].x + " unified x: " + x);
								}
                                if (isBackCamera)
                                    x = -x;
...
//								Log.d(TAG, "detect face point : orientation: " + orientation + " x: " + (pointf[0]) + "  y: " + (pointf[1]));
...
//							Log.d(TAG, "detect face rect : left: " + (faces[c].rect.left) + "  top: " + (faces[c].rect.top) + " height: " + height + " width: " + width);

							//test kyle
//							float faceX = (faces[c].rect.left * 1.0f/height)*2 - 1;
//							if (isBackCamera)
//								faceX = -faceX;
//							float faceY = (faces[c].rect.top * 1.0f / width) * 2 - 1;
//							float[] pointFacef = new float[]{faceY, faceX, 0.0f};
//							if (orientation == 1)
//								pointFacef = new float[]{-faceY, -faceX, 0.0f};
//							if (orientation == 2)
//								pointFacef = new float[]{faceY, -faceX, 0.0f};
//							if (orientation == 3)
//								pointFacef = new float[]{-faceX, -faceY, 0.0f};
//							Log.d(TAG, "detect face rect : x: " + (pointFacef[0]) + "  y: " + (pointFacef[1]));
//
//							FloatBuffer fbFace = mCameraMatrix.floatBufferUtil(pointFacef);
//							pointsFaceLeftTop.add(fbFace);
							//end
                    ...
                    synchronized (mPointsMatrix) {
						...
						//kyle
						mPointsMatrix.faceRectLeftTops = pointsFaceLeftTop;
                    }
...
//        }
		...
		sglRender.onSurfaceCreated(gl, config);
		sglRenderBackGround.onSurfaceCreated(gl, config);
    }
    ...
    public void onSurfaceChanged(GL10 gl, int width, int height) {
		...
		sglRender.onSurfaceChanged(gl, width, height);
		sglRenderBackGround.onSurfaceChanged(gl, width, height);
    }
...
//        mPointsMatrix.draw(mMVPMatrix);
		...
		sglRenderBackGround.onDrawFrame(gl, 0);
		sglRender.onDrawFrame(gl, -noseCenterX);
    }
    ...
    public void onClickStart(View view){
		...
		//停顿2s后播放画外音
		NarratorSpeak.startPlay1(new MediaPlayer.OnCompletionListener() {
			@Override
